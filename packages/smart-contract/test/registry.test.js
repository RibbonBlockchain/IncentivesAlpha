...
describe("Registry tests", async () => {
  ...
  beforeEach("", async () => {
    ...
    adminInstance = await deployer.deploy(AdminAbi, false);
    ...
    await (await vaultInstance
      .from(admin)
      .init(registryInstance.contract.address)).wait();
  });
    ...
    describe("Killing contract", async () => {
      it("Killing contract as Admin contract", async () => {
        ...
        assert.equal(
          activeState,
          false,
          "Contract has not been killed successfully"
        );
      });
    });
  });
      ...
      it("Adding a user as a non-admin fails", async () => {
        try {
          ...
          assert.equal(
            true,
            false,
            "User was able to add a user with incorrect permissions"
          );
        } catch (error) {
          assert.equal(
            true,
            true,
            "User was unable to add a user without correct permissions"
          );
        }
      ...
      it("Adding a user after contract has been killed fails", async () => {
        ...
        try {
          ...
          assert.equal(
            true,
            false,
            "User was able to add a user with a killed contract"
          );
        } catch (error) {
          assert.equal(
            true,
            true,
            "User was unable to add a user with a killed contract"
          );
        }
      ...
      it("Removing a user after contract has been killed fails", async () => {
        ...
        try {
          ...
          assert.equal(
            true,
            false,
            "User was able to remove a user with a killed contract"
          );
        } catch (error) {
          assert.equal(
            true,
            true,
            "User was unable to remove a user with a killed contract"
          );
        }
      ...
      it("Updating a user as a non admin fails", async () => {
        ...
        try {
          ...
          assert.equal(
            true,
            false,
            "User was able to update a user with incorrect permissions"
          );
        } catch (error) {
          assert.equal(
            true,
            true,
            "User was unable to update a user without correct permissions"
          );
        }
      ...
      it("Updating a user after contract has been killed fails", async () => {
        ...
        try {
          ...
          assert.equal(
            true,
            false,
            "User was able to update a user with a killed contract"
          );
        } catch (error) {
          assert.equal(
            true,
            true,
            "User was unable to update a user with a killed contract"
          );
        }
  ...
  describe("View functions", async () => {
    ...
    it("Verifying payout", async () => {
      try {
        let userRoll = await registryInstance
          .from(admin)
          .verifyPayout(admin.signer.address);
        assert.equal(
          true,
          false,
          "Incorect address can call protected function"
        );
      } catch (error) {
