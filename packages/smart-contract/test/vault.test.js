...
describe("Vault tests", async () => {
  ...
  describe("State changing functions", async () => {
    describe("Donating funds", async () => {
      it("Adding funds through donation", async () => {
        ...
        await vaultInstance
          .from(admin)
          .donateFunds("for the betterment of SA healthcare", {
            value: amount
          });
        ...
        assert.equal(
          balance.toString(),
          amount.toString(),
          "Balance of vault incorrect, sending funds failed"
        );
      });
    ...
    describe("Making a payout", async () => {
      it("Making a payout as an admin", async () => {
        ...
        assert.equal(
          balance.toString(),
          0,
          "Vault did not correctly send funds"
        );
      ...
      it("Making a payout as a non admin", async () => {
        ...
        try {
          ...
          assert.equal(
            true,
            false,
            "User was able to create payout with incorrect permissions"
          );
        } catch (error) {
          assert.equal(
            true,
            true,
            "User was not able to send payout without correct permissions"
          );
        }
  ...
  describe("View functions", async () => {
    it("Getting user roles", async () => {
      ...
      let unregisteredRoll = await registryInstance.getUserRole(
        admin2.signer.address
      );
